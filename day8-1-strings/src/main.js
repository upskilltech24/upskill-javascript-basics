...
function isPalindromeString() {
  ...
  const enteredInput = input2.value;
  if (enteredInput === "") {
    alert("Enter a valid string");
  } else {
    for (let count = 0; count < enteredInput.length / 2; count++) {
      if (
        enteredInput[count] !== enteredInput[enteredInput.length - 1 - count]
      ) {
        alert("Entered string is not a palindrome");
        return;
      }
    }
    alert("Entered string is a palindrome string");
  }
}

function printFirstHalfInUpperCase() {
  ...
  if (enteredInput === "") {
    ...
    // ABCDEFGHIJ
    // for (let count = 0; count < enteredInput.length; count++) {
    //   if (count < enteredInput.length / 2) {
    //     newString += enteredInput[count].toUpperCase();
    //   } else {
    //     newString += enteredInput[count].toLowerCase();
    //   }

    const part1 = enteredInput.substring(0, enteredInput.length / 2);
    const part2 = enteredInput.substring(
      enteredInput.length / 2,
      enteredInput.length
    );
    newString = part1.toUpperCase() + part2.toLowerCase();
    document.getElementById("printString").innerHTML = newString;
  }
}
